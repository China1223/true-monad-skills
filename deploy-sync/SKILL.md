...
| 文件 | 内容 | 谁写入 | 格式 |
| --- | --- | --- | --- |
| `VERSION` | Git 代码版本号 | `--git` | `v8.0.4-0722-0042` |
| `RELEASE_VERSION` | Release 发行版本号 | `--release` | `v8.0.4-0722-0042-0722A` |

两个文件**互不干扰**：`--git` 只写 VERSION，`--release` 只写 RELEASE_VERSION。

### Release 部署版本号递增

Release 部署使用 `--release` 参数：读 VERSION 获取 Git 版本号，追加 Release 标记，写入 RELEASE_VERSION。
...
# 首次部署：RELEASE_VERSION = v8.0.4-0722-0042-0722A
# 同日再次部署：RELEASE_VERSION = v8.0.4-0722-0042-0722B
# 次日部署：RELEASE_VERSION = v8.0.4-0722-0042-0723A
# 新 Git 提交后首次部署：RELEASE_VERSION = v8.0.4-0723-0043-0723A
```

### 版本号格式拆解

```
RELEASE_VERSION: v8.0.4-0722-0042-0722A
                   │      │    │    │
                   │      │    │    └── Release 标记：日期(MMDD) + 字母(A/B/C.../AA/AB...)
                   │      │    └── Git 全局序号(4位，永不重置)
                   │      └── Git 提交日期(MMDD)
                   └── 大版本号
```
...
| 操作 | 参数 | 写入文件 | 说明 |
| --- | --- | --- | --- |
| Git 管理 | `--git` | `VERSION` | 序号+1，不动 RELEASE_VERSION |
| Release 部署 | `--release` | `RELEASE_VERSION` | 读 VERSION，追加 `-{Release日期}{字母}` |

> `--git` 后字母不继承，下次 Release 从 A 开始。详见 `deploy/maintenance/bump_version.py`。
...
| 最新代码版本号 | 根目录 `VERSION` 文件 |
| 最新发行版本号 | 根目录 `RELEASE_VERSION` 文件（若存在） |
...
**版本号格式**：
- 打包 Release：读取 `RELEASE_VERSION` 文件内容
- 源码部署：读取 `VERSION` 文件内容 + ` 源码`
